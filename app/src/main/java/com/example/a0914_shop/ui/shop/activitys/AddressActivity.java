package com.example.a0914_shop.ui.shop.activitys;
...
import com.example.a0914_shop.presenter.cart.AdressPersenter;
...
public class AddressActivity extends BaseActivity<ICart.IAdressPersenter> implements ICart.IAdressView {
    @BindView(R.id.ed_name)
    EditText edName;
    @BindView(R.id.ed_psd)
    EditText edPsd;
    @BindView(R.id.ed_ads)
    EditText edAds;
    ...
    private void openAdressSelect() {
        if (mPopWindow != null && mPopWindow.isShowing()) {
            ...
            //完成
            txt_submit = contentView.findViewById(R.id.txt_submit);
            txt_submit.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    mPopWindow.dismiss();
                    mPopWindow = null;
                    //点击完成之后将选择好的地址赋值给地址编辑框
                    //省份
                    String tvProvince = txtProvince.getText().toString().trim();
                    Log.d(TAG, "onClick: " + tvProvince);
                    //城市
                    String tvcity = txtCity.getText().toString().trim();
                    //区域
                    String tvarea = txtArea.getText().toString().trim();
                    //把选择好的地址赋值给编辑框
                    edAds.setText(tvProvince + "," + tvcity + "," + tvarea);
                }
            });
            mPopWindow.showAtLocation(lls, Gravity.BOTTOM, 0, 0);
